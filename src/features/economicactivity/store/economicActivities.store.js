...
import economicActivitiesService from '../services/economicActivities.service.js';



export const useEconomicActivitiesStore = defineStore('economicActivities', {
    state: () => ({
        items: [],
        selectedItem: null,
        loading: false,
    ...
    actions: {
        ...
        async fetchItems() {
            return this._run(async () => {
                const response = await economicActivitiesService.list({
        ...
        async loadCatalogs() {
            try {
                this.catalogs = await economicActivitiesService.getFormOptions();
        ...
        createItem(data) {
            return this._run(
                () => economicActivitiesService.create(data).then(r => this._afterCreate(r)),
        ...
        updateItem(uuid, data) {
            return this._run(async () => {
                const response = await economicActivitiesService.update(uuid, data);
        ...
        async deleteItem(uuid) {
            return this._run(async () => {
                await economicActivitiesService.delete(uuid);
        ...
        getProfile(uuid) {
            return this._run(async () => {
                const r = await economicActivitiesService.getProfile(uuid);
        ...
        fetchProfileById(uuid) {
            return this._run(async () => {
                const r = await economicActivitiesService.get(uuid);
                this.selectedItem = r?.data?.data ?? r?.data ?? r;
                return this.selectedItem;
