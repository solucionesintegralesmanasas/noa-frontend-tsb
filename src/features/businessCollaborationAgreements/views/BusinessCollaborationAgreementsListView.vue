<template>
    <BasePageHeader :title="'Listado de ' + 'BusinessCollaborationAgreements'"
        description="Gestión del módulo en el sistema." icon="fad fa-clipboard-list text-primary" :show-refresh="true"
        :show-create="true" :show-bg="true" :loading="isViewLoading || store.loading" :compact="true"
        :breadcrumbs="[{ label: 'BusinessCollaborationAgreements' }, { label: 'Listado' }]" @refresh="refreshTable"
        @create="goToCreate" />
                ...
                <div v-else class="card-body p-0">
                    <div class="table-responsive scrollbar">
                        <DataTable :value="store.items" lazy :paginator="true" :rows="store.pagination.itemsPerPage"
                            ...
                            <Column field="agreement_internal_id" header="ID Interno" sortable>
                                <template #body="{ data }">
                                    <span class="text-dark">{{ data.agreement_internal_id || '-' }}</span>
                                </template>
                            </Column>

                            <Column field="contracting_entity_name" header="Entidad" sortable>
                                <template #body="{ data }">
                                    <span class="text-dark">{{ data.contracting_entity_name || '-' }}</span>
                                </template>
                            </Column>
...
const goToCreate = () => router.push('/convenios-colaboracion/crear');
const goToEdit = (uuid) => router.push(`/convenios-colaboracion/editar/${uuid}`);

const handleDelete = (item) => confirmDelete(item, {
    title: '¿Eliminar convenio?',
    nameField: 'agreement_internal_id',
});

const downloadPdf = async (uuid) => {
    try {
        downloadingPdf.value = uuid;
        await BusinessCollaborationAgreementsService.downloadPdf(uuid);
    } catch (error) {
        console.error(error);
