<template>
    <BasePageHeader :title="'Listado de ' + 'Información Tributaria'"
        description="Gestión de información tributaria y fiscal de las empresas."
        ...
        :breadcrumbs="[{ label: 'Información Tributaria' }, { label: 'Listado' }]" @refresh="refreshTable"
        @create="goToCreate" />
                ...
                <div v-if="isViewLoading" class="card-body p-0">
                    <div class="table-responsive scrollbar">
                        <table class="table table-sm mb-0">
                            <thead>
                                <tr>
                                    <th v-for="w in ['25%', '20%', '20%', '15%', '10%', '10%']" :key="w"
                ...
                <div v-else class="card-body p-0">
                    <div class="table-responsive scrollbar">
                        <DataTable :value="store.items" lazy :paginator="true" :rows="store.pagination.itemsPerPage"
                            ...
                            <Column field="company_uuid" header="Empresa" sortable>
                                <template #body="{ data }">
                                    <span class="fw-medium text-dark">
                                        {{ data.company?.business_name || data.company?.name || '—' }}
                                    </span>
                                </template>
                            </Column>

                            <Column field="is_withholding_agent_exempt" header="Agente Retención" sortable
                                class="text-center" style="width: 140px;">
                                <template #body="{ data }">
                                    <span class="badge rounded-pill badge-subtle" :class="data.is_withholding_agent_exempt
                                            ? 'badge-subtle-success'
                                            : 'badge-subtle-warning'
                                        ">
                                        <i :class="data.is_withholding_agent_exempt
                                                ? 'fad fa-check-circle me-1'
                                                : 'fad fa-times-circle me-1'
                                            " style="font-size:10px;" />
                                        {{ data.is_withholding_agent_exempt ? "Exento" : "Agente" }}
                                    </span>
                                </template>
                            </Column>

                            <Column field="tax_special_regime" header="Régimen Especial" sortable>
                                <template #body="{ data }">
                                    <span class="text-muted small">
                                        {{ data.tax_special_regime || '—' }}
                                    </span>
                                </template>
                            </Column>

                            <Column field="company_size" header="Tamaño" sortable class="text-center"
                                style="width: 130px;">
                                <template #body="{ data }">
                                    <span v-if="data.company_size"
                                        class="badge bg-info bg-opacity-10 text-info fw-semibold">
                                        {{ capitalizeSize(data.company_size) }}
                                    </span>
                                    <span v-else class="text-muted">—</span>
                                </template>
                            </Column>

                            <Column field="financial_statements_path" header="Estados Financieros" class="text-center"
                                style="width: 120px;">
                                <template #body="{ data }">
                                    <a v-if="data.financial_statements_path" :href="data.financial_statements_path"
                                        target="_blank" class="btn btn-falcon-default btn-sm" title="Ver archivo">
                            ...
                            <Column header="Acciones" class="text-center" style="min-width:120px; width: 120px;">
                                <template #body="{ data }">
...
const capitalizeSize = (size) => {
    ...
    return map[size] || size.charAt(0).toUpperCase() + size.slice(1);
};
...
const goToCreate = () => router.push('/empresas/informacion-tributaria/crear');
const goToEdit = (uuid) => router.push(`/empresas/informacion-tributaria/editar/${uuid}`);

const handleDelete = (item) => confirmDelete(item, {
    title: '¿Eliminar información tributaria?',
    nameField: 'company.business_name',
});
