<template>
    <BasePageHeader :title="'Listado de ' + 'Datos Bancarios'"
        description="Gestión de información bancaria corporativa." icon="fad fa-university text-primary"
        :show-refresh="true" :show-create="true" :show-bg="true" :loading="isViewLoading || store.loading"
        :compact="true" :breadcrumbs="[{ label: 'Datos Bancarios' }, { label: 'Listado' }]" @refresh="refreshTable"
        @create="goToCreate" />
                ...
                <div v-if="isViewLoading" class="card-body p-0">
                    <div class="table-responsive scrollbar">
                        <table class="table table-sm mb-0">
                            <thead>
                                <tr>
                                    <th v-for="w in ['20%', '20%', '20%', '15%', '15%', '10%']" :key="w"
                ...
                <div v-else class="card-body p-0">
                    <div class="table-responsive scrollbar">
                        <DataTable :value="store.items" lazy :paginator="true" :rows="store.pagination.itemsPerPage"
                            ...
                            <Column field="bank_name" header="Banco" sortable>
                                <template #body="{ data }">
                            ...
                            <Column field="account_number" header="Número de Cuenta" sortable>
                                <template #body="{ data }">
                                    <code class="text-primary small">{{ data.account_number || '—' }}</code>
                                </template>
                            </Column>

                            <Column field="account_type" header="Tipo" sortable>
                                <template #body="{ data }">
                                    <span class="badge bg-info bg-opacity-10 text-info fw-semibold">
                                        {{ data.account_type ? capitalize(data.account_type) : '—' }}
                                    </span>
                                </template>
                            </Column>

                            <Column field="account_holder" header="Titular" sortable>
                                <template #body="{ data }">
...
const goToCreate = () => router.push('/empresas/datos-bancarios/crear');
const goToEdit = (uuid) => router.push(`/empresas/datos-bancarios/editar/${uuid}`);
const goToDetail = (uuid) => router.push(`/empresas/datos-bancarios/perfil/${uuid}`);

const handleDelete = (item) => confirmDelete(item, {
    title: '¿Eliminar cuenta bancaria?',
    nameField: 'account_number',
});
