...
export const useBankDetailsStore = defineStore('bankDetails', {
    state: () => ({
        items: [],
        selectedItem: null,
        loading: false,
    ...
    actions: {
        ...
        async fetchItems() {
            return this._run(async () => {
                const response = await bankDetailsService.list({
        ...
        async loadCatalogs() {
            try {
                this.catalogs = await bankDetailsService.getFormOptions();
        ...
        createItem(data) {
            return this._run(
                () => bankDetailsService.create(data).then(r => this._afterCreate(r)),
        ...
        updateItem(uuid, data) {
            return this._run(async () => {
                const response = await bankDetailsService.update(uuid, data);
        ...
        async deleteItem(uuid) {
            return this._run(async () => {
                await bankDetailsService.delete(uuid);
        ...
        getProfile(uuid) {
            return this._run(async () => {
                const r = await bankDetailsService.getProfile(uuid);
        ...
        fetchProfileById(uuid) {
            return this._run(async () => {
                const r = await bankDetailsService.get(uuid);
                this.selectedItem = r?.data?.data ?? r?.data ?? r;
                return this.selectedItem;
