...
export const useBranchesStore = defineStore('branches', {
    ...
    actions: {
        ...
        async loadCatalogs() {
            try {
                this.catalogs = await branchesService.getFormOptions();
        ...
        createItem(data) {
            return this._run(
                () => branchesService.create(data).then(r => this._afterCreate(r)),
        ...
        updateItem(uuid, data) {
            return this._run(async () => {
                const response = await branchesService.update(uuid, data);
        ...
        async deleteItem(uuid) {
            return this._run(async () => {
                await branchesService.delete(uuid);
        ...
        getProfile(uuid) {
            return this._run(async () => {
                const r = await branchesService.getProfile(uuid);
        ...
        fetchProfileById(uuid) {
            return this._run(async () => {
                const r = await branchesService.get(uuid);
                this.selectedItem = r?.data?.data ?? r?.data ?? r;
                return this.selectedItem;
